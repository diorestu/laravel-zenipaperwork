...
<div class="space-y-5">
    <div class="flex items-center justify-between">
        <h1 class="text-lg font-semibold text-gray-900">Rekening Bank</h1>
    ...
    <section class="rounded-lg border border-gray-200 bg-white p-5">
        <form method="GET" action="{{ route('settings.bank-accounts') }}" class="grid gap-3 sm:grid-cols-5">
            <input name="search" value="{{ request('search') }}" placeholder="Cari rekening" class="rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-gray-900 focus:outline-none">
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse ($bankAccounts as $account)
                        ...
                        <x-ui.modal name="edit-bank-account-{{ $account->id }}" :is-open="(int) request('edit') === $account->id" class="max-w-xl p-6">
                            <h2 class="mb-4 text-lg font-semibold text-gray-900">Edit Rekening Bank</h2>
    ...
    <x-ui.modal name="create-bank-account" :is-open="request('modal') === 'create'" class="max-w-xl p-6">
        <h2 class="mb-4 text-lg font-semibold text-gray-900">Tambah Rekening Bank</h2>
