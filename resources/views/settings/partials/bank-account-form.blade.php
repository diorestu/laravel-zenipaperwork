<form method="POST" action="{{ $action }}" class="space-y-3">
    ...
    <x-form.select name="bank_name" label="Bank Name" :options="$bankOptions" :value="$account?->bank_name">
        <option value="">Pilih bank</option>
    </x-form.select>
    <x-form.input name="account_name" label="Account Name" :value="$account?->account_name" />
    <x-form.input name="account_number" label="Account Number" :value="$account?->account_number" />
    <x-form.input name="branch" label="Branch" :value="$account?->branch" />
    <x-form.input name="currency" label="Currency" :value="$account?->currency ?? 'IDR'" />
