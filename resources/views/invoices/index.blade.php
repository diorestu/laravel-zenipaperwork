...
<div class="space-y-6">
    ...
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h1 class="text-xl font-semibold text-gray-900 dark:text-white/90">Invoices</h1>
        ...
        <section class="grid grid-cols-2 gap-4 xl:grid-cols-4">
            @foreach ($invoiceStats as $stat)
                <div class="rounded-lg border border-gray-200 bg-white p-[0.85rem] shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03]">
                    ...
                    <p class="mt-1 text-xs text-gray-400 dark:text-gray-500">{{ $stat['meta'] }}</p>
                </div>
            @endforeach
        </section>
    @endif
    ...
    <x-ui.modal name="create-invoice" :is-open="request('modal') === 'create'" class="max-w-5xl p-6">
        <h2 class="mb-4 text-lg font-semibold text-gray-900 dark:text-white/90">Create Invoice</h2>
