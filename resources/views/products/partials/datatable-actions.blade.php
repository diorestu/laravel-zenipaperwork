...
<div class="flex items-center justify-center gap-1.5">
    ...
    <form method="POST" action="{{ route('products.destroy', $product) }}" onsubmit="return confirm('Hapus product ini?')">
        @csrf
        @method('DELETE')
        <button class="inline-flex h-8 w-8 items-center justify-center rounded-lg text-error-500 hover:bg-error-50 hover:text-error-600 dark:text-error-400 dark:hover:bg-error-500/10" aria-label="Delete product">
            <svg width="16" height="16" viewBox="0 0 20 20" fill="none" aria-hidden="true">
