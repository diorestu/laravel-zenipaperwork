...
<aside id="sidebar"
    class="fixed flex flex-col mt-0 top-0 px-3 left-0 bg-white dark:bg-gray-900 dark:border-gray-800 text-gray-900 h-screen transition-all duration-300 ease-in-out z-99999 border-r border-gray-200"
    x-data="{
        openSubmenus: {},
        init() {
    ...
    :class="{
        ...
        'w-[78px]': !$store.sidebar.isExpanded && !$store.sidebar.isHovered,
    ...
    <div class="shrink-0 pt-5 pb-4 flex"
        ...
        <a href="/" class="flex h-10 items-center overflow-hidden">
            ...
            <span
                x-show="!$store.sidebar.isExpanded && !$store.sidebar.isHovered && !$store.sidebar.isMobileOpen"
                class="block h-9 w-9 overflow-hidden rounded-lg"
    ...
    <!-- Navigation Menu -->
    <div class="min-h-0 flex-1 overflow-y-auto pb-4 duration-300 ease-linear no-scrollbar">
        <nav>
            <div class="flex flex-col gap-3">
                @foreach ($menuGroups as $groupIndex => $menuGroup)
                    <div>
                        <!-- Menu Group Title -->
                        <h2 class="mb-2.5 px-2 text-[11px] font-semibold uppercase flex leading-[18px] text-gray-400"
                            :class="(!$store.sidebar.isExpanded && !$store.sidebar.isHovered && !$store.sidebar.isMobileOpen) ?
                                ...
                                <li>
                                    @if (isset($item['subItems']))
                                        ...
                                        <div x-show="isSubmenuOpen({{ $groupIndex }}, {{ $itemIndex }}) && ($store.sidebar.isExpanded || $store.sidebar.isHovered || $store.sidebar.isMobileOpen)">
                                            <ul class="mt-1.5 space-y-1 ml-8">
                                                @foreach ($item['subItems'] as $subItem)
                                                    <li>
