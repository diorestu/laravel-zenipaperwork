...
<form method="POST" action="{{ $action }}" class="space-y-5 rounded-lg border border-gray-200 bg-white p-5" x-data="itemForm({ productData: {{ $productJson }}, existingItems: {{ $itemsJson }} })">
    ...
    <div class="grid gap-4 sm:grid-cols-2">
        <x-form.select name="client_id" label="Client" :value="$quotation?->client_id">
            @foreach ($clients as $client)
                <option value="{{ $client->id }}" @selected((int) old('client_id', $quotation?->client_id) === $client->id)>{{ $client->name }}</option>
            @endforeach
        </x-form.select>
        <x-form.input name="number" label="Number" :value="$quotation?->number ?? 'QUO-'.now()->format('Ymd-His')" />
        <x-form.input name="issue_date" label="Issue Date" type="date" :value="optional($quotation?->issue_date)->format('Y-m-d') ?? now()->toDateString()" />
        <x-form.input name="valid_until" label="Valid Until" type="date" :value="optional($quotation?->valid_until)->format('Y-m-d')" />
        <x-form.input name="tax_rate" label="Tax Rate" type="number" step="0.01" :value="$quotation?->tax_rate ?? 0" />
    </div>
    ...
    <button class="rounded-md bg-gray-900 px-4 py-2 text-sm font-medium text-white cursor-pointer">Save Quotation</button>
</form>
