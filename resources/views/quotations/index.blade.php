...
<div class="space-y-6">
    ...
    <section class="rounded-lg border border-gray-200 bg-white shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03]">
        <form method="POST" action="{{ route('quotations.bulk-delete') }}" x-data="{ hasSelection: false }" @change="hasSelection = document.querySelectorAll('.dt-checkbox:checked').length > 0">
            @csrf
            <div class="border-b border-gray-100 p-5 dark:border-gray-800">
                <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                    <div class="flex items-center gap-3">
                        <template x-if="hasSelection">
                            <button type="submit" onclick="return confirm('Apakah Anda yakin ingin menghapus penawaran yang dipilih?')" class="inline-flex items-center justify-center rounded-lg bg-error-500 px-3 py-1.5 text-xs font-medium text-white shadow-theme-xs hover:bg-error-600">
                                Hapus Terpilih
                            </button>
                        </template>
                    </div>
