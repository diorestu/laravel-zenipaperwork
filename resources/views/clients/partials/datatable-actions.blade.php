...
<div class="flex items-center justify-center gap-1.5">
    <button type="button" class="js-edit-record inline-flex h-8 w-8 items-center justify-center rounded-lg text-gray-500 hover:bg-gray-50 hover:text-brand-600 dark:text-gray-400 dark:hover:bg-white/[0.03] dark:hover:text-brand-400" data-modal="edit-client" data-record-payload="{{ $recordPayload }}" aria-label="Edit klien">
    ...
    <form method="POST" action="{{ route('clients.destroy', $client) }}" onsubmit="return confirm('Hapus client ini?')">
        @csrf
        @method('DELETE')
        <button class="inline-flex h-8 w-8 items-center justify-center rounded-lg text-error-500 hover:bg-error-50 hover:text-error-600 dark:text-error-400 dark:hover:bg-error-500/10" aria-label="Hapus klien">
            <svg width="16" height="16" viewBox="0 0 20 20" fill="none" aria-hidden="true">
